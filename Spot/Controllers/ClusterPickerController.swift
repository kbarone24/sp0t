...
class ClusterPickerController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if imageObjects.isEmpty { return 0 } else {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let imageObject = imageObjects[safe: indexPath.row] {
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                cell.requestID = self.imageManager.requestImage(for: imageObject.0.asset, targetSize: self.baseSize, contentMode: .aspectFill, options: self.options) { (result, _) in
    ...
    func getTitle() {
        ...
        CLGeocoder().reverseGeocodeLocation(tappedLocation, preferredLocale: locale) { placemarks, _ in // 6
