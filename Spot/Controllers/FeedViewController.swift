...
class FeedViewController: UIViewController {
            ...
            if let index = info["index"] as? Int {
                ...
                if index > (activePosts.count - 4) && self.refresh == .yesRefresh {
                    ...
                    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                        ...
                        if self.selectedSegmentIndex == 0 { self.getFriendPosts(refresh: false) } else if self.activeRadius < 1_000 { self.activeRadius *= 2; self.getNearbyPosts(radius: self.activeRadius) }
                    }
                }
            }
        }
    ...
    func getFriendPosts(refresh: Bool) {
        ...
        self.listener1 = query.addSnapshotListener(includeMetadataChanges: true, listener: { [weak self] (snap, _) in
        ...
        for post in posts {
            ...
            if !friendPosts.contains(where: { $0.id == post.id }) {
                ...
                if !mapVC.deletedPostIDs.contains(post.id ?? "") && !mapVC.deletedFriendIDs.contains(post.posterID) {
                    ...
                    let newPost = friendPosts.count > 10 && post.timestamp.seconds > friendPosts.first?.timestamp.seconds ?? 100_000_000_000
    ...
    func getNearbyPosts(radius: Double) {
        ...
        if circleQuery == nil {
            ...
            _ = circleQuery?.observeReady {
                ...
                if self.nearbyEnteredCount == 0 && self.activeRadius < 1_024 {
    ...
    func loadPostFromDB(key: String?, location: CLLocation?) {
        ...
        listener5 = ref.addSnapshotListener({ [weak self] (doc, _) in
            ...
            do {
                ...
                if !self.currentNearbyPosts.contains(where: { $0.id == postInfo.id }) { self.currentNearbyPosts.append(postInfo) } else if let i = self.currentNearbyPosts.firstIndex(where: { $0.id == postInfo.id }) { self.currentNearbyPosts[i] = postInfo; if !escaped { self.noAccessCount += 1 }; escaped = true; }
                self.accessEscape()
    ...
    func accessEscape() {
        if noAccessCount + currentNearbyPosts.count == nearbyEnteredCount && queryReady {
            if currentNearbyPosts.count < 10 && activeRadius < 1_000 { /// force reload if active radius reaches 1024
    ...
    func getPostScore(post: MapPost) -> Double {
        ...
        let factor = Double(min(1 + (100_000_000 / timeSincePost), 2_000))
            ...
            for doc in commentSnap!.documents {
                do {
                    ...
                    if let i = tempFriends.firstIndex(where: { $0.id == commentInfo.commenterID }) {
                        ...
                        self.db.collection("users").document(commentInfo.commenterID).getDocument { (snap, _) in
    ...
    func checkForLocationChange() {
        ...
        if postVC != nil && postVC.tableView != nil && selectedSegmentIndex == 1 && activeRadius < 1_000 {
