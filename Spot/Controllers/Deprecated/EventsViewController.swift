...
class EventsViewController: UIViewController, UIGestureRecognizerDelegate, UITabBarControllerDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    let eventsScroll: UICollectionView = UICollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout.init())
    let eventsLayout: UICollectionViewFlowLayout = UICollectionViewFlowLayout.init()
    
    var popularScroll: UICollectionView = UICollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout.init())
    var botPicksScroll: UICollectionView = UICollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout.init())
    var simpleLayout: UICollectionViewFlowLayout = UICollectionViewFlowLayout.init()
    ...
    override func viewDidLoad() {
        ...
        let gesture = UIPanGestureRecognizer.init(target: self, action: #selector(EventsViewController.panGesture))
            ...
            docLoop: for doc in spotSnapshot!.documents {
                if let privacyLevel = doc.get("privacyLevel") as? String {
                    ...
                    if self.region.contains(CLLocationCoordinate2D(latitude: spotLat, longitude: spotLong)) {
                        ...
                        self.listener3 = self.db.collection("spots").document(doc.documentID).collection("events").addSnapshotListener({ (eventsSnap, err) in
                ...
                if let privacyLevel = doc.get("privacyLevel") as? String {
                    ...
                    if let spotPicURL = doc.get("imageURL") as? String {
                        ...
                        if self.region.contains(CLLocationCoordinate2D(latitude: spotLat, longitude: spotLong)) {
                            ...
                            let range : TimeInterval = 1209600
                            ...
                            self.listener5 = query.addSnapshotListener({ (feedSnap, err) in
 ...
 /*
    ...
    */
    ...
    func loadBotPicks() {
        var botList: [String] = []
        self.listener7 = db.collection("botpicks").addSnapshotListener({ (botSnap, err) in
            var count = 0
    ...
    func getBotInfo(bots: [String]) {
        for bot in bots {
            self.listener8 = db.collection("spots").document(bot).addSnapshotListener{ (snapshot, err) in
    ...
    func getCity() {
        ...
        CLGeocoder().reverseGeocodeLocation(self.currentLocation) { placemarks, error in // 6
...
extension EventsViewController: CLLocationManagerDelegate {
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        self.region = CLCircularRegion(center: CLLocationCoordinate2D(latitude: lat, longitude: long), radius: 20000, identifier: "region")
        ...
        if (eventsList.count == 0) {
            ...
            if !(self.startingLocation.coordinate.latitude == 0.0) {
                let regionCheck = CLCircularRegion(center: CLLocationCoordinate2D(latitude: lat, longitude: long), radius: 5000, identifier: "region")
