...
class CreatePostViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate {
        ...
        listener1 = self.db.collection("users").document(self.uid).addSnapshotListener { (snapshot, err) in
            if let err = err {
                ...
                if self.profilePicURL == "" {
                    ...
                    gsReference.getData(maxSize: 1 * 2048 * 2048) {
                        data, error in
                        if error != nil {
                            print("error occured")
                        } else {
    ...
    */
    ...
    @objc func handleCreatePost(_sender: AnyObject){
      ...
      //  self.tabBarController?.tabBar.isUserInteractionEnabled = false
        ...
        self.listener2 = self.db.collection("users").document(self.uid).addSnapshotListener { (snapshot, userErr) in
            ...
            self.listener3 = self.db.collection("users").document(self.uid).collection("spotsList").addSnapshotListener { (spots, userErr) in
    ...
    func uploadPostImage(_ images:[UIImage], postId: String, postValues: [String: Any], completion: @escaping ((_ url:String?) -> ())){
                    ...
                    storageRef.downloadURL(completion: { url, error in
                        ...
                        if index == imageCount {
                            ...
                            if !self.selectedUsers.isEmpty {
                                self.db.collection("users").document(self.spotObject.founder).getDocument { (fSnap, err) in
    ...
    func triggerDestruct(postID: String) {
        ...
        postRef.collection("Comments").getDocuments{ (commentsnapshot, err) in
