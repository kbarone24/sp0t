...
class MapTarget: UIView {
    ...
    func resizeBanner(width: CGFloat) {
        ...
        spotBannerBackground.frame = CGRect(x: 0, y: spotBannerBackground.frame.minY + 2, width: self.frame.width, height: spotBannerBackground.frame.height)
        spotBannerBackground.layer.cornerRadius = 7.5
        
        spotNameLabel.frame = CGRect(x: 5, y: spotNameLabel.frame.minY + 2, width: self.frame.width - 10, height: 16)
        
        targetIcon.frame = CGRect(x: 100 - widthAdjustment/2 + 6.25, y: 38.5, width: 27.5, height: 34)
        bannerTriangle.frame = CGRect(x: 108 - widthAdjustment/2 + 8.3, y: 47.8, width: 7.2, height: 5.75)
