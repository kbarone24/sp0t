...
struct RenderSettings {
    ...
    var size = CGSize(width: UIScreen.main.bounds.width * 2, height: 1200)
...
class ImageAnimator {
    ...
    init(renderSettings: RenderSettings, images: [UIImage]) {
        var rotatedImages: [UIImage] = []
        for image in images {
            let imgsize = image.size;
            UIGraphicsBeginImageContext(imgsize)
            image.draw(in: CGRect(x: 0.0, y: 0.0, width: imgsize.width, height: imgsize.height))
            let correctedImage = UIGraphicsGetImageFromCurrentImageContext();
            rotatedImages.append(correctedImage ?? UIImage())
            UIGraphicsEndImageContext();
        }
        settings = renderSettings
        videoWriter = VideoWriter(renderSettings: settings)
        self.images = rotatedImages
    }
